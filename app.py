...
def help_box(text: str) -> None:
    ...
    with st.expander("Section guidance"):
        st.write(text)
...
def abstract_notes_tab() -> None:
    ...
    st.write("Paste the abstract and keep a short summary of the paper.")
    help_box(
    ...
    update_paper(
        "one_sentence_summary",
        st.text_area("One-sentence paper summary", value=paper["one_sentence_summary"], height=100),
    )
    update_paper(
        "background_information",
...
def research_question_tab() -> None:
    st.header("4. Purpose and Aim Summary")
    st.write("Summarise the purpose of the study without turning it into a test.")
    help_box(
        "This section captures the study focus, aim, expected finding, and biomedical importance in plain language."
    )

    paper = st.session_state.paper
    update_paper("problem_studied", st.text_area("Research problem or background issue", value=paper["problem_studied"]))
    update_paper("research_question", st.text_area("Central study focus", value=paper["research_question"]))
    update_paper("study_aim", st.text_area("Study aim", value=paper["study_aim"]))
    update_paper("hypothesis", st.text_area("Expected finding", value=paper["hypothesis"]))
    update_paper("research_importance", st.text_area("Biomedical importance", value=paper["research_importance"]))
    ...
    for index in range(5):
        ...
        with st.expander(f"Result {index + 1}", expanded=index == 0):
            ...
            update_result(
                index,
                "explanation",
                st.text_area("Simple result summary", value=result["explanation"], key=f"explanation_{index}"),
            )


def discussion_tab() -> None:
    st.header("7. Discussion and Conclusion")
    st.write("Summarise the meaning of the results and the main message of the paper.")
    help_box("The discussion section explains the meaning of the results. The conclusion gives the main message of the paper.")

    paper = st.session_state.paper
    update_paper("results_meaning", st.text_area("Meaning of the results", value=paper["results_meaning"]))
    update_paper("answer_to_question", st.text_area("Link between results and study focus", value=paper["answer_to_question"]))
    update_paper("main_conclusion", st.text_area("Main conclusion", value=paper["main_conclusion"]))
    update_paper(
        "comparison_previous_studies",
        st.text_area("Comparison with previous studies", value=paper["comparison_previous_studies"]),
    )
    update_paper("biomedical_importance", st.text_area("Biomedical science importance", value=paper["biomedical_importance"]))
    update_paper("real_life_use", st.text_area("Real-life usefulness", value=paper["real_life_use"]))
    update_paper("personal_understanding", st.text_area("Student-friendly interpretation", value=paper["personal_understanding"]))


def limitations_tab() -> None:
    st.header("8. Limitations Summary")
    st.write("Summarise the limits of the study and the improvements suggested by the paper.")
    help_box("Limitations show what the study can and cannot prove. They help keep the summary accurate.")

    paper = st.session_state.paper
    update_paper("common_limitations", st.multiselect("Common limitations checklist", LIMITATIONS, default=paper["common_limitations"]))
    update_paper("author_limitations", st.text_area("Author-stated limitations", value=paper["author_limitations"]))
    update_paper("student_limitations", st.text_area("Additional limitations", value=paper["student_limitations"]))
    update_paper("questions_still_have", st.text_area("Revision notes", value=paper["questions_still_have"]))
    update_paper("future_improvements", st.text_area("Future research improvements", value=paper["future_improvements"]))
...
def vocabulary_tab() -> None:
    ...
    help_box("A good vocabulary note is short, accurate, and easy to reuse while revising the paper.")
